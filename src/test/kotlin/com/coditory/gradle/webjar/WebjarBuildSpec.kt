...
class WebjarBuildSpec {
    ...
    fun `should dynamically add additional files to task inputs`() {
        val inputFiles = listOf("src/index.js", ".babelrc", "package.json", "package-lock.json")
        val project = project()
            .withFiles(inputFiles)
            .withPlugins(WebjarPlugin::class)
            .build()
        val buildTask = project.getNpmTask(WEBJAR_BUILD_TASK)
        assertThat(buildTask.inputs.files).containsAll(
            inputFiles.map { project.projectDir.resolve(it) }
        )
    }

    @Test
    fun `should configure webjarBuild task to run before build task`() {
        val buildTask = project.getNpmTask(WEBJAR_BUILD_TASK)
        val javaBuildTask = project.getTask(BUILD_TASK_NAME)
        assertThat(javaBuildTask.dependsOn).contains(buildTask)
    }
